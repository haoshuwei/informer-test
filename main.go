		...
		cache.FilteringResourceEventHandler{
			...
			Handler: cache.ResourceEventHandlerFuncs{
				...
				UpdateFunc: func(objA, objB interface{}) {
					...
					ips1 := make([]string, 0)
					ips2 := make([]string, 0)
					for _, address := range ep1.Subsets[0].Addresses {
						ips1 = append(ips1, address.IP)
					}
					for _, address := range ep2.Subsets[0].Addresses {
						ips2 = append(ips2, address.IP)
					}
					//klog.Infof("UpdateEp, name:%s, oldEp:%v, newEp:%v", ep1.Name, ips1, ips2)
					klog.Infof("UpdateEp, name:%s, newEp:%v", ep1.Name, ips2)
...
func main() {
	...
	if home := homedir.HomeDir(); home != "" {
		...
		kubeconfigTemp = flag.String("kubeconfig", kubeconfigPath, "(optional) absolute path to the kubeconfig file")
	} else {
		kubeconfigTemp = flag.String("kubeconfig", "", "absolute path to the kubeconfig file")
	}
	flag.Parse()
